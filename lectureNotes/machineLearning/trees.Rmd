...
\center
![](treeFigs/heartAttack.png){width=75%}
...
## Decision Trees
A tree is basically a flow chart of yes or no questions. The general idea of the methods we are describing is to define an algorithm that uses data to create these trees with predictions at the ends, referred to as __nodes__.


\vskip .2in
Regression and decision trees, or sometimes called **Classification and Regression Trees (CART)**, operate by predicting an outcome variable $Y$ by partitioning the predictors ${\bf X}$.

## Regression trees

When the \underline{outcome is continuous}, we call the method a __regression tree__. We will use polling data to estimate the conditional expectation $f(x) = \mbox{E}(Y | X = x)$ with $Y$ the poll margin and $x$ the day. 
...
## Regression trees
The general idea here is to build a decision tree and, at the end of each __node__, obtain a predictor $\hat{y}$. 
...
\vskip .2in
But how do we decide on the partition  $R_1, R_2, \ldots, R_J$ and how do we choose $J$?
...
## Regression trees
After we define the new partitions $R_1$ and $R_2$, we compute predictors by taking the average of all the observations $y$ for which the associated $\mathbf{x}$ is in $R_1$ and $R_2$. We refer to these two as $\hat{y}_{R_1}$ and $\hat{y}_{R_2}$ respectively. 
...
## Regression trees
Note that the algorithm stopped partitioning at 8. Now we explain how this decision is made. Every time we split and define two new partitions, our training set RSS decreases because our model has more flexibility to adapt to the training data. 
...
## Regression trees
However, cp is not the only parameter used to decide if we should partition a current partition or not. Another common parameter is the minimum number of observations required in order to split a partition. The argument used in the `rpart` function is `minsplit` and the default is 20. 
...
**Classification trees**, or **decision trees**, are used in prediction problems where the \underline{outcome is categorical}. We use the same partitioning principle with some differences to account for the fact that we are now working with a categorical outcome. 
...
\vskip .1in
Second, is that we can no longer use RSS to choose the partition. While we could use the naive approach of looking for partitions that minimize training error, better performing approaches use more sophisticated metrics. Two of the more popular ones are the __Gini Index__ and __Entropy__.   
...
## Classification (decision) trees
Lets look at how a classification tree performs on the digits example we examined before (no perfect solution this time):
